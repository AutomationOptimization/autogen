...
DEFAULT_ANCHOR_TEMPLATE = (
    ...
    "**Do NOT allow any repetitive patterns occur in your final output.**\n"
    " **ESCP can contain actual words or tokens, but those tokens must NOT be present in the given user's message.**"
    "Let 𝐑 ∈ ℝ^{V×d} be the static embedding matrix, 𝐒⊆ℝ^d the user-prompt\n"
    "subspace, and 𝐀⊆ℝ^d the anchor subspace.  We construct triads τᵢ consisting\n"
    "of three rare tokens {t₁,t₂,t₃} with ‖𝐑τᵢ‖₂ ≥ 3.3 and pairwise cos ≤ 0.05.\n"
...
class TSCEChat:
    def __init__(self, model: str | None = None, *, anchor_prompt: str = DEFAULT_ANCHOR_TEMPLATE, final_prefix: str = DEFAULT_FINAL_PREFIX, deployment_id: str | None = None, client: openai.BaseClient | callable | None = None):
    ...
    def _normalize_chat(self, prompt_or_chat: Union[str, Chat]) -> Chat:
        ...
        if isinstance(prompt_or_chat, Sequence):
            ...
            if not all(isinstance(m, dict) and "role" in m and "content" in m for m in prompt_or_chat):
    ...
    def __call__(self, prompt_or_chat: Union[str, Chat], *, anchor_temp: float = 1.6) -> "TSCEReply":
        ...
        anchor_resp = self._completion(anchor_msg, temperature=1.7, top_p=0.01, max_tokens=500, stop=["</ESCP>"])
        anchor_text = anchor_resp["choices"][0]["message"]["content"].strip()
        anchor_model = anchor_resp.get("model")
        final_sys_content = SECOND_PASS_BRIEF + anchor_text + "##END Embedding Space Control Prompt##\n" + "Continue with primary directive below:\n\n" + DEFAULT_FINAL_PREFIX
        final_msg: Chat = [{"role": "system", "content": final_sys_content}] + chat
        final_resp = self._completion(final_msg, temperature=0.01, top_p=1.0, logprobs=LOGPROB, top_logprobs=5 if LOGPROB else None)
        ...
        lp: list = []
        if LOGPROB:
            choice_dict = final_resp["choices"][0]
        ...
        reply = TSCEReply(content=final_text, anchor=anchor_text, anchor_model=anchor_model, final_model=final_model)
        reply.logprobs = lp
        return reply

    def _completion(self, messages: List[dict[str, str]], **gen_kwargs):
        if self.backend == "ollama":
            ...
            resp = self.client.chat(model=model, messages=messages, stream=False, options=options or None)
    ...
    def _completion_anchor(self, messages: List[dict[str, str]], **gen_kwargs):
        if self.backend == "ollama":
            model = self.model or self._auto_deployment or "llama3"
            mapping = {"temperature": "temperature", "top_p": "top_p", "max_tokens": "num_predict"}
            options = {mapping[k]: v for k, v in gen_kwargs.items() if k in mapping}
...
class TSCEReply:
    def __init__(self, *, content: str, anchor: str, anchor_model: str | None = None, final_model: str | None = None):
    ...
    def __repr__(self):
        return f"TSCEReply(content={self.content!r}, anchor={self.anchor!r}, anchor_model={self.anchor_model!r}, final_model={self.final_model!r})"
